...
def analyseSeriesCircuit(k,series_output):
    ...
    for i in range(len(k)):
          ...
          PHI =math.degrees(math.atan((ZL-ZC)/R)) if R != 0 else 0
          series_output.append([round(ZL,1),round(ZC,1),round(ZT,1),round(A,1),round(PHI,1)])
...
def analyseParallelCircuit(k,parallel_output):
    ...
    for i in range(len(k)):
          ...
          ZL=2*math.pi*F*L*0.001
          ZC=(1/(2*math.pi*F*C*0.000001))
          if ZL != 0 and ZC != 0 and R != 0:
            ZT_ = math.sqrt((1/R**2) + (((1/ZL)-(1/ZC))**2))
            ZT = 1/ZT_
...
for i in range(len(no_of_circuits)):
    if no_of_circuits[i][0]=="series":
        series_circuits.append(no_of_circuits[i])
for i in range(len(no_of_circuits)):
    if no_of_circuits[i][0]=="parallel":
        parallel_circuits.append(no_of_circuits[i])

series_output=[]    
parallel_output=[]
analyseSeriesCircuit(series_circuits,series_output)
analyseParallelCircuit(parallel_circuits,parallel_output)
result=[]
s=0
p=0
for i in range(len(no_of_circuits)):
